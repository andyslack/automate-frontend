...
<div class="col-span-12">
    <div class="intro-y overflow-auto lg:overflow-visible mt-2">
        ...
        <div v-if="user.user_type >= 9">
            <div style="float:right;" v-if="status_options != undefined && type_options != undefined && assignee_options != undefined">
                <form name="selectform">
                    <select name="status" id="status" class="input input--lg box w-full lg:w-auto mt-3 mr-3 lg:mt-0 ml-auto" v-model="filterStatus" @change="submitfilter()">
                        <option hidden>Status</option>
                        <option>All</option>
                        <option v-for="option in status_options" :key="option">{{option}}</option>
                    </select>

                    <select name="sku_category" id="sku_category" class="input input--lg box w-full lg:w-auto mt-3 mr-3 lg:mt-0 ml-auto" v-model="filterSku" @change="submitfilter()">
                        <option hidden>Type</option>
                        <option>All</option>
                        <option v-for="option in type_options" :key="option">{{option}}</option>
                    </select>

                    <select name="assignee" id="assignee" class="input input--lg box w-full lg:w-auto mt-3 mr-3 lg:mt-0 ml-auto" v-model="filterAssignee" @change="submitfilter()">
        ...
        <table v-if="tasks.length > 0" class="table table-report sm:mt-2">
            ...
            <tbody>
                <tr class="intro-x" v-for="task in tasks" :key="task._id">
                    ...
                    <td class="w-30">
                        <a v-if="task" v-bind:href="'/task/' + task._id">{{getTitle(task)}}</a>
                    ...
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" v-bind:href="'/task/' + task._id">
                                <search-icon class="w-4 h-4 mr-1"></search-icon>View
                            </a>
                            <a v-if="task.task_status === 'Pending' && task.sku.sku_bill_per_word" href="javascript:;" data-toggle="modal" data-target="#split-task-<%=task._id%>" class="flex items-center mr-3 color-info">
                                <share-2-icon class="w-4 h-4 mr-1"></share-2-icon>Split
                            </a>
                            <a v-if="task.task_status === 'Complete' || task.task_status === 'Archived'" href="#" class="flex items-center mr-3 color-danger" onclick="removeTask('<%=task._id%>');return false;">
                                <trash-2-icon class="w-4 h-4 mr-1"></trash-2-icon>Remove
                            </a>
                        </div>
                    ...
                    <div class="modal" :id="`split-task-${task._id}`">
                        <div class="modal__content">
                            ...
                            <div class="px-5 pb-8 text-center">
                                ...
                                <button type="button" class="button button-info w-24 text-white" onclick="return split_onclick('qty-<%=task._id%>', document.getElementById('qty-<%=task._id%>').value, '<%=task._id%>')">Split Task</button>
                            </div>
        ...
        <div v-if="tasks.length == 0 && user.user_type < 9" class="col-span-12 md:col-span-6 xl:col-span-12 xxl:col-span-12 xl:col-start-1 xl:row-start-1 xxl:col-start-auto xxl:row-start-auto mt-3">
            <div class="intro-x">
                <div class="box">
                    <div class="p-5">
                        ...
                        <div class="text-gray-600 text-justify mt-1">
                            <p>
                                ...
                                <a href="https://automate.mn/link-building/#pricing" target="_blank">links</a>,
                                <a href="https://automate.mn/content/#pricing" target="_blank">content</a> or
                                <a href="https://automate.mn/translations/#pricing" target="_blank">translations</a> and we will automatically link them to your account.
                                <br />
                                <br />If you have made a purchase and it is not showing
                                <a href="mailto:support@automate.mn" target="_blank">contact us</a> and we will investigate.
                            </p>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    mounted() {
        console.log("here tasks", this.status_options)
    },
    computed: {},
    methods: {
        submitfilter() {
                ...
                switch (task.sku.sku_category) {
                    case "Content":
                        if (
                            ...
                            typeof task.project_details !== undefined
                        ) {
                            if (
                                typeof task.project_details !== undefined &&
